...
object Anagrams {
  ...
  def wordAnagrams(word: Word): List[Word] = wordsMatch(wordOccurrences(word))
  ...
  def combinations(occurrences: Occurrences): List[Occurrences] = {
    occurrences :: (for( occ <- occurrences ) yield combinations(wordsUnmatch(occurrences, occ._1))).flatten.distinct
  }
  ...
  def subtract(x: Occurrences, y: Occurrences): Occurrences = {
    if( y.length == 0 ) x
      ...
      val ch = y.head._1
      subtract(wordsUnmatch(x, ch), wordsUnmatch(y, ch))
    }
  }
  ...
  def wordsMatch(occ: Occurrences): List[Word] = {
  ...
  def wordsUnmatch(occurrences: Occurrences, ch: Char): Occurrences = {
